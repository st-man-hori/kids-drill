...
const sleevedTorsoPath = (sleeveLen: number): string => {
  ...
  // 肩(neckL〜sleeveTopL)は、えりから腕へ向けて連続的に下っていく
  // 「なで肩」のラインにしている。えりのすぐ外側を一旦フラットにしてから
  // 肩の先だけ下げる形だと、そのフラットな区間が「肩が張っている」ように
  // 見えてしまい、実機フィードバックで「変わってない」と指摘された。
  // また、そでを覆うためだけの頂点(そでのはば18.5〜32.5の外、x13付近)を
  // 高く保つ必要は無いのに以前はそこも持ち上げていて、肩の先だけ上がって
  // 見える不自然な跳ね上がりになっていた(実機フィードバックの画像で確認)。
  // そでを覆う高さが要るのはそでのはばの範囲内(x18.5〜32.5、ここでは
  // shoulderの頂点(32,58)がその役目を持つ)だけなので、そこから外側
  // (x13付近)は逆に下げ続けてよく、途中で高さが変わらない区間も
  // 上向きに跳ねる区間も作らず、えりから腕の先まで一本の下り坂になる
  // よう頂点を置き直している。以前は肩の外側を袖の上端(SLEEVE_TOP_Y)より
  // 上まで持ち上げて覆っており、それが「いかり肩(ドラゴンボールみたい)」
  // に見える原因になっていた(実機フィードバック)。SLEEVE_TOP_Yをうで
  // rect自体の上端(y60)近くまで下げたことで、肩を持ち上げなくても動く
  // 袖を覆えるようになっている
  //
  ...
  const points: [number, number][] = [
    [40, 54], // えりの左はし(首の下に隠れる)
    [30, 57], // 肩(そでのはばの内側。ここでそでの上端を覆う)
    [19, 61], // 腕へ向けてさらに下る
    [12, 65], // 腕の先まで下り続ける(跳ね上げない)
...
const sleevedCoatPath = (sleeveLen: number): string => {
  ...
  const points: [number, number][] = [
    [40, 54], // えりの左はし(首の下に隠れる)
    [30, 57], // 肩(そでのはばの内側。sleevedTorsoPathのコメント参照)
    [19, 61], // 腕へ向けてさらに下る
    [12, 65], // 腕の先まで下り続ける(跳ね上げない)
    [sleeveOuterBottomX, capBottom - 9], // 袖のそとがわ下(手首側、丈に応じて絞る)
