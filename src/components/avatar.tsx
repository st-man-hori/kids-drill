...
const sleevedTorsoPath = (sleeveLen: number): string => {
  ...
  // 途中で高さが変わらない区間を作らず、えりから腕まで一本の下り坂に
  // なるよう頂点を置き直している。以前は肩の外側を袖の上端(SLEEVE_TOP_Y)
  // より上まで持ち上げて覆っており、それが「いかり肩(ドラゴンボール
  // みたい)」に見える原因になっていた(実機フィードバック)。SLEEVE_TOP_Y
  // をうでrect自体の上端(y60)近くまで下げたことで、肩を持ち上げなくても
  // 動く袖を覆えるようになっている
  //
  ...
  const points: [number, number][] = [
    ...
    [32, 58], // 肩からすでに下りはじめる
    [21, 60], // 腕へ向けてさらに下る
    [13, 54], // そでの上端付近へのつなぎ(そでを覆うため一度持ち上げる)
...
const sleevedCoatPath = (sleeveLen: number): string => {
  ...
  const points: [number, number][] = [
    [40, 54], // えりの左はし(首の下に隠れる)
    [32, 58], // 肩からすでに下りはじめる(sleevedTorsoPathのコメント参照)
    [21, 60], // 腕へ向けてさらに下る
    [13, 54], // そでの上端付近へのつなぎ
    [sleeveOuterBottomX, capBottom - 9], // 袖のそとがわ下(手首側、丈に応じて絞る)
