...
サービス名は「さんすうチャレンジ」（リポジトリ名・開発コードネームは`kids-drill`のまま）。算数ドリルの学習Webアプリ。小学1年生のたし算・ひき算から始め、小学1〜6年生の算数まで拡張予定（他教科は現時点では計画なし。理由はarchitecture.md参照）。タブレット・スマホファースト（メインターゲットはiPad）、PCにも対応。リポジトリはパブリック公開予定。
