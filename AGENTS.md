...
サービス名は「キッズドリルゲーム」（ドメイン`kids-drill.st-man.com`、リポジトリ名`kids-drill`）。算数ドリルの学習Webアプリ。小学1年生のたし算・ひき算から始め、小学1〜6年生の算数まで拡張予定。**名前に教科を入れていないのは意図的**（他教科への拡張余地を残すため。漢字のよみを含む検討状況はarchitecture.md参照）。タブレット・スマホファースト（メインターゲットはiPad）、PCにも対応。リポジトリはパブリック公開予定。
