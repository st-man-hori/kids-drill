...
サービス名は「キッズドリルゲーム」（ドメイン`kids-drill-game.st-man.com`、リポジトリ名`kids-drill`）。小学生向けの学習Webアプリ。算数（小学1年生のたし算・ひき算）から始め、対応教科・学年を順次拡張予定（現状の対応教科は下記「現在の状態」参照）。**名前に教科を入れていないのは意図的**（他教科への拡張余地を残すため。経緯はarchitecture.md参照）。タブレット・スマホファースト（メインターゲットはiPad）、PCにも対応。リポジトリはパブリック公開予定。
...
認証（サインアップ・ログイン）、算数の練習モード（たしざん）、かんじよみクイズ（`/practice/kanji`、小1〜4対応）、タイムアタックモード、ランキング、着せ替え（アイテムの解放・ポイント交換・装備）が動く状態。着せ替えのアバターは実イラストがまだ無くSVG図形のダミー。タイムアタックはプロトタイプ実装で、出題難易度は練習モードと同じ`child_progress`の現在レベルを流用しており、専用の難易度調整は未検討。テストはVitest + Testing Libraryでセットアップ済み。
